...
2.  **Código del Backend (Archivo de Configuración de Google Strategy):**
    ...
    - **Código Corregido (Ejemplo):** `callbackURL: 'http://localhost:3000/api/auth/google/callback'`
      _(Para producción, reemplazar por `https://[tu-dominio-de-produccion].com/api/auth/google/callback`)_
...
## Actualización: Integración GET /api/portfolios (Backend → Frontend)

**Fecha:** 2026-07-30

### Objetivo

Conectar el endpoint `GET /api/portfolios` del backend NestJS con la página `/dashboard/portfolios` del frontend, reemplazando los datos mock por llamadas reales a la API.

### Problema

La tabla `PortfoliosTable` mostraba datos mock (16 portfolios hardcodeados) en vez de los portfolios reales del usuario autenticado.

### Análisis de desajustes Backend vs Frontend

| Aspecto              | Backend real                                                             | Frontend (antes)                                              |
| -------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------- |
| Estructura respuesta | `{ success, message, data: { items, total, page, limit, totalPages } }`  | `{ data: [], meta: { page, pageSize, total, totalPages } }`   |
| Campos Portfolio     | `id, name, slug, description, isPublic, publicUrl, createdAt, updatedAt` | `id, name, template, status, createdAt, updatedAt, publicUrl` |
| Paginación           | `limit` (dentro de `data`)                                               | `pageSize` (dentro de `meta`)                                 |

### Archivos modificados

#### 1. `src/types/portfolio.ts`

- **Cambio:** Interfaz `Portfolio` actualizada para alinearse con el backend
- **Eliminados:** `template` y `status` (no existen en el backend)
- **Agregados:** `slug`, `description`, `isPublic`
- **Nuevo tipo:** `PortfolioListApiResponse` que modela la respuesta exacta del backend
  ```typescript
  interface PortfolioListApiResponse {
    success: boolean;
    message: string;
    data: {
      items: Portfolio[];
...
#### 2. `src/lib/services/portfolio.ts`

- **Cambio:** Reemplazado servicio mock completo por llamadas reales via `http`
- **Eliminados:** Array de 16 portfolios mock, función `delay()`
- **Implementado:**
  - `getPortfolios(page, pageSize)` — `GET /api/portfolios?page=X&limit=Y` con mapeo de respuesta
  - `getPortfolioById(id)` — `GET /api/portfolios/:id`
  - `deletePortfolio(id)` — `DELETE /api/portfolios/:id`
- **Mapeo:** La respuesta del backend (`data.items`, `data.limit`) se transforma al formato `PaginatedResponse<Portfolio>` (`data`, `meta.pageSize`)

#### 3. `src/components/dashboard/PortfoliosTable.tsx`

- **Cambio:** Columnas y badges actualizados
- **Eliminada:** Columna `template` (no existe en backend)
- **Cambiada:** Columna `status` → `isPublic` con badge "Público"/"Privado"
- **Badge config:** `{ true: { label: 'Público', className: '...' }, false: { label: 'Privado', className: '...' } }`

#### 4. `src/lib/api/http.ts`

- **Cambio:** Agregado `cache: 'no-store'` para requests del cliente
- **Problema que resuelve:** El navegador cacheaba respuestas del API (304 Not Modified) en vez de hacer llamadas reales al backend
- **Línea:** `cache: isServer ? options.cache : 'no-store'`

#### 5. `src/app/dashboard/portfolios/page.tsx`

- **Cambio 1:** Agregado `toastError` para feedback visual de errores
- **Cambio 2:** Eliminado warning de ESLint (`react-hooks/exhaustive-deps`)
  - El `useEffect` inicial ahora llama `getPortfolios(1, 5)` directamente con valores fijos
  - `router` agregado al array de dependencias
- **Cambio 3:** Bloque `catch` mejorado con `toastError("No se pudieron cargar los portafolios")`

### Archivos modificados (Backend)

#### 6. `prisma/migrations/20260602011910_create_password_reset_token_table/migration.sql`

- **Problema:** Directorio vacío (sin archivo SQL) bloqueaba aplicar migraciones pendientes
- **Solución:** Creado archivo `migration.sql` con no-op (`SELECT 1;`)
- **Acción:** `npx prisma migrate resolve --applied` para marcar como aplicadas

### Problemas encontrados y resueltos

#### Problema 1: Tabla `portfolios` no existía en la BD

**Síntoma:** Empty state siempre visible, API retornaba datos vacíos
**Causa:** 2 migraciones pendientes (`create_password_reset_token_table` y `add_portfolios`)
**Solución:** Marcar migraciones como aplicadas con `prisma migrate resolve`

#### Problema 2: Navegador cacheaba respuestas del API (304)

**Síntoma:** `GET /api/portfolios` retornaba 304 Not Modified en vez de datos reales
**Causa:** El HTTP client no desactivaba el cache por defecto
**Solución:** Agregar `cache: 'no-store'` en `http.ts` para requests del cliente

#### Problema 3: Errores silenciosos en la UI

**Síntoma:** La página mostraba empty state sin indicar qué fallaba
**Causa:** El bloque `catch` solo hacía `console.error`
**Solución:** Agregar `toastError` para feedback visual

#### Problema 4: Warning ESLint `react-hooks/exhaustive-deps`

**Síntoma:** `pagination.page` y `pagination.pageSize` faltaban en dependencias del useEffect
**Causa:** El effect leía estado que actualizaba, creando dependencia circular
**Solución:** Llamar `getPortfolios(1, 5)` directamente con valores fijos en el effect inicial

### Flujo de datos implementado

```
1. Usuario navega a /dashboard/portfolios
2. page.tsx → useEffect → portfolioService.getPortfolios(1, 5)
3. portfolioService → http.get('/api/portfolios?page=1&limit=5')
   → cookies httpOnly se envían automáticamente (credentials: 'include')
4. Backend retorna { success, message, data: { items, total, page, limit, totalPages } }
5. portfolioService mapea → { data: items, meta: { page, pageSize, total, totalPages } }
6. page.tsx pasa datos a <PortfoliosTable>
7. DataTable renderiza tabla con paginación
```
...
- ✅ TypeScript: Sin errores
- ✅ ESLint: Sin errores nuevos
- ✅ Mapeo de tipos alineado con backend
- ✅ Cache deshabilitado para llamadas API
- ✅ Feedback de errores visible con toast

### Estado actual

La integración del frontend con el backend está **completada a nivel de código**. Para funcionar se requiere:

1. Que el usuario tenga una sesión activa (login con email/password o Google)
2. Que existan portfolios en la BD para el usuario autenticado
3. Reiniciar el backend después de aplicar migraciones para regenerar Prisma client

### Pendiente

- Verificar que el login funcione correctamente (el usuario reportó 401 en `/api/auth/me`)
- Crear un portfolio desde el frontend para probar el flujo completo
